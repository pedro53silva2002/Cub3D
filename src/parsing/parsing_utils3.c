...
/// @return Returns 0 if it finds some hole. Returns 1 if it finds nothing wrong
int ft_has_hole(char **map, int x, int y, int height)
{
	//ft_printf("X: %d\t Y: %d\tHeight: %d\n", x , y, height);
	if ((x + 1 == (int)ft_strlen(map[y]) || !map[y][x + 1]) || (map[y][x + 1] != '1' && map[y][x + 1] != 'N' && map[y][x + 1] != 'S' && map[y][x + 1] != 'E' && map[y][x + 1] != 'W' && map[y][x + 1] != '0'))
		return (1);
	if ((x == 0 || !map[y][x - 1])|| (map[y][x - 1] != '1' && map[y][x - 1] != 'N' && map[y][x - 1] != 'S' && map[y][x - 1] != 'E' && map[y][x - 1] != 'W' && map[y][x - 1] != '0'))
		return (1);
	if ((y + 1 == height || !map[y + 1][x]) || (map[y + 1][x] != '1' && map[y + 1][x] != 'N' && map[y + 1][x] != 'S' && map[y + 1][x] != 'E' && map[y + 1][x] != 'W' && map[y + 1][x] != '0'))
		return (1);
	if ((y == 0 || !map[y - 1][x]) || (map[y - 1][x] != '1' && map[y - 1][x] != 'N' && map[y - 1][x] != 'S' && map[y - 1][x] != 'E' && map[y - 1][x] != 'W' && map[y - 1][x] != '0'))
		return (1);
	return (0);
}
...
/// @param tmp_map The temporary map
/// @param c If it "x" it will give the coordinate x of player. If "y" it will give the player's y coordinate
...
int ft_get_coor(char **tmp_map, char c)
{
	...
	while (tmp_map[++i])
	{
		...
		while (tmp_map[i][++j])
		{
			if (tmp_map[i][j] == 'N' || tmp_map[i][j] == 'S' || tmp_map[i][j] == 'E' || tmp_map[i][j] == 'W')
			{
				if (c == 'x')
					coor = j;
				else if (c == 'y')
...
int ft_check_unkchr(char **map)
{
	...
	while (map[++i])
	{
		...
		while (map[i][++j])
		{
			if (map[i][j] != '0' && map[i][j] != 'N' && map[i][j] != 'S' && map[i][j] != 'E' && map[i][j] != 'W' && map[i][j] != '1' && map[i][j] != ' ')
			{
				return (0);
			}
		}
